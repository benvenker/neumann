...
Tests enforce pages-first defaults, `pages.jsonl` emission with HTTP `uri`s, syntax highlighting behavior, summary constraints, and Chroma collection setup. Unit tests for metadata normalization (`tests/test_metadata_normalization.py`) verify `_normalize_metadata_for_chroma()` converts lists to comma-separated strings and `_parse_meta_list()` rehydrates them correctly.
